...
abstract contract DelegateERC20 is ERC20 {
    ...
    function delegateBySig(
        ...
        require(signatory != address(0), "OKswapToken::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "OKswapToken::delegateBySig: invalid nonce");
        require(now <= expiry, "OKswapToken::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint256 blockNumber) external view returns (uint256) {
        require(blockNumber < block.number, "OKswapToken::getPriorVotes: not yet determined");
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(block.number, "OKswapToken::_writeCheckpoint: block number exceeds 32 bits");
...
contract OKswapToken is DelegateERC20, Ownable {
    uint256 private constant preMineSupply = 500000 * 1e18;
    uint256 private constant maxSupply = 98000000 * 1e18; // the total supply

    using EnumerableSet for EnumerableSet.AddressSet;
    EnumerableSet.AddressSet private _minters;

    constructor() public ERC20("OKSwap Token", "OK") {
    ...
    function addMinter(address _addMinter) public onlyOwner returns (bool) {
        require(_addMinter != address(0), "OKswapToken: _addMinter is the zero address");
        return EnumerableSet.add(_minters, _addMinter);
    }

    function delMinter(address _delMinter) public onlyOwner returns (bool) {
        require(_delMinter != address(0), "OKswapToken: _delMinter is the zero address");
    ...
    function getMinter(uint256 _index) public view onlyOwner returns (address) {
        require(_index <= getMinterLength() - 1, "OKswapToken: index out of bounds");
...
// File: interfaces/IOKswapPair.sol


pragma solidity >=0.5.0;

interface IOKswapPair {
    event Approval(address indexed owner, address indexed spender, uint256 value);
    event Transfer(address indexed from, address indexed to, uint256 value);
...
// File: interfaces/IOKswapFactory.sol
...
// File: libraries/OKswapLibrary.sol
...
library OKswapLibrary {
    using SafeMathOKswap for uint256;

    // returns sorted token addresses, used to handle return values from pairs sorted in this order
    function sortTokens(address tokenA, address tokenB) internal pure returns (address token0, address token1) {
        require(tokenA != tokenB, "OKswapLibrary: IDENTICAL_ADDRESSES");
        (token0, token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), "OKswapLibrary: ZERO_ADDRESS");
    }
            ...
            uint256(
                keccak256(
                    abi.encodePacked(
                        ...
                        hex"4b335476687e6467d3ae424605b3d9aba8fd9596429170ef188b79f93170c909" // init code hash
                    )
                )
            )
        );
    ...
    function getReserves(
        ...
        (uint256 reserve0, uint256 reserve1, ) = IOKswapPair(pairFor(factory, tokenA, tokenB)).getReserves();
    ...
    function quote(
        ...
        require(amountA > 0, "OKswapLibrary: INSUFFICIENT_AMOUNT");
        require(reserveA > 0 && reserveB > 0, "OKswapLibrary: INSUFFICIENT_LIQUIDITY");
    ...
    function getAmountOut(
        ...
        require(amountIn > 0, "OKswapLibrary: INSUFFICIENT_INPUT_AMOUNT");
        require(reserveIn > 0 && reserveOut > 0, "OKswapLibrary: INSUFFICIENT_LIQUIDITY");
    ...
    function getAmountIn(
        ...
        require(amountOut > 0, "OKswapLibrary: INSUFFICIENT_OUTPUT_AMOUNT");
        require(reserveIn > 0 && reserveOut > 0, "OKswapLibrary: INSUFFICIENT_LIQUIDITY");
    ...
    function getAmountsOut(
        ...
        require(path.length >= 2, "OKswapLibrary: INVALID_PATH");
...
// Dependency file: contracts/interfaces/IAsset.sol
...
contract TradingPool is Ownable {
    ...
    struct UserInfo {
        ...
        uint256 accOkAmount; // How many rewards the user has got.
    }
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. oks to distribute per block.
        uint256 lastRewardBlock; // Last block number that oks distribution occurs.
        uint256 accOkPerShare; // Accumulated oks per share, times 1e12.
        uint256 quantity;
        uint256 accQuantity;
        uint256 allocOkAmount;
        uint256 accOkAmount;
    }
    ...
    // The OK Token!
    OKswapToken public ok;
    // ok tokens created per block.
    uint256 public okPerBlock;
    // Info of each pool.
    ...
    // factory address
    IOKswapFactory public factory;
    // The block number when ok mining starts.
    ...
    function getOkPerBlock(uint256 blockNumber) public view returns (uint256) {
        uint256 _phase = phase(blockNumber);
        return okPerBlock.div(2**_phase);
    }

    function getOkBlockReward(uint256 _lastRewardBlock) public view returns (uint256) {
        ...
        while (lastRewardPhase < currentPhase) {
            ...
            blockReward = blockReward.add((height.sub(_lastRewardBlock)).mul(getOkPerBlock(height)));
            _lastRewardBlock = height;
        }
        blockReward = blockReward.add((block.number.sub(_lastRewardBlock)).mul(getOkPerBlock(block.number)));
        return blockReward;
    }
    ...
    // Update the given pool's ok allocation point. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        bool _withUpdate
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 blockReward = getOkBlockReward(pool.lastRewardBlock);

        if (blockReward <= 0) {
            return;
        }

        uint256 okReward = blockReward.mul(pool.allocPoint).div(totalAllocPoint);

        bool minRet = ok.mint(address(this), okReward);
        if (minRet) {
            pool.accOkPerShare = pool.accOkPerShare.add(okReward.mul(1e12).div(pool.quantity));
            pool.allocOkAmount = pool.allocOkAmount.add(okReward);
            pool.accOkAmount = pool.accOkAmount.add(okReward);
        }
        pool.lastRewardBlock = block.number;
    }
    ...
    function swap(
        ...
        address pair = OKswapLibrary.pairFor(address(factory), input, output);
        ...
        if (user.quantity > 0) {
            uint256 pendingReward = user.quantity.mul(pool.accOkPerShare).div(1e12).sub(user.rewardDebt);
            if (pendingReward > 0) {
        ...
        user.rewardDebt = user.quantity.mul(pool.accOkPerShare).div(1e12);
        emit Swap(account, pairOfPid[pair], quantity);

        return true;
    }

    function pendingOk(uint256 _pid, address _user) public view returns (uint256) {
        require(_pid <= poolInfo.length - 1, "TradingPool: Can not find this pool");

        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accOkPerShare = pool.accOkPerShare;

        if (user.quantity > 0) {
            if (block.number > pool.lastRewardBlock) {
                uint256 blockReward = getOkBlockReward(pool.lastRewardBlock);
                uint256 okReward = blockReward.mul(pool.allocPoint).div(totalAllocPoint);
                accOkPerShare = accOkPerShare.add(okReward.mul(1e12).div(pool.quantity));
                return user.pendingReward.add(user.quantity.mul(accOkPerShare).div(1e12).sub(user.rewardDebt));
            }
            if (block.number == pool.lastRewardBlock) {
                return user.pendingReward.add(user.quantity.mul(accOkPerShare).div(1e12).sub(user.rewardDebt));
            }
    ...
    function withdraw(uint256 _pid) public {
        ...
        updatePool(_pid);
        uint256 pendingAmount = pendingOk(_pid, tx.origin);

        if (pendingAmount > 0) {
            safeOkTransfer(tx.origin, pendingAmount);
            pool.quantity = pool.quantity.sub(user.quantity);
            pool.allocOkAmount = pool.allocOkAmount.sub(pendingAmount);
            user.accOkAmount = user.accOkAmount.add(pendingAmount);
            user.quantity = 0;
            user.rewardDebt = 0;
    ...
    function emergencyWithdraw(uint256 _pid) public {
        ...
        safeOkTransfer(msg.sender, user.pendingReward);

        pool.quantity = pool.quantity.sub(user.quantity);
        pool.allocOkAmount = pool.allocOkAmount.sub(user.pendingReward);
        user.accOkAmount = user.accOkAmount.add(user.pendingReward);
        user.quantity = 0;
        user.rewardDebt = 0;
        user.pendingReward = 0;
    ...
    // Safe ok transfer function, just in case if rounding error causes pool to not have enough oks.
    function safeOkTransfer(address _to, uint256 _amount) internal {
        uint256 okBalance = ok.balanceOf(address(this));
        if (_amount > okBalance) {
            ok.transfer(_to, okBalance);
        } else {
            ok.transfer(_to, _amount);
        }
    }

    // Set the number of ok produced by each block
    function setOkPerBlock(uint256 _newPerBlock) public onlyOwner {
        massUpdatePools();
        okPerBlock = _newPerBlock;
    }
    ...
    function getPoolView(uint256 pid) public view returns (PoolView memory) {
        ...
        ERC20 token0 = ERC20(IOKswapPair(pair).token0());
        ERC20 token1 = ERC20(IOKswapPair(pair).token1());
        ...
        uint256 rewardsPerBlock = pool.allocPoint.mul(okPerBlock).div(totalAllocPoint);
        return
            PoolView({
                pid: pid,
                pair: pair,
                ...
                accOkPerShare: pool.accOkPerShare,
                rewardsPerBlock: rewardsPerBlock,
                allocOkAmount: pool.allocOkAmount,
                accOkAmount: pool.accOkAmount,
    ...
    function getUserView(address pair, address account) public view returns (UserView memory) {
        ...
        uint256 unclaimedRewards = pendingOk(pid, account);
        return
            UserView({
                quantity: user.quantity,
                accQuantity: user.accQuantity,
                unclaimedRewards: unclaimedRewards,
                accOkAmount: user.accOkAmount
            });
    }
...
// Root file: contracts/pools/LiquidityPool.sol
...
contract LiquidityPool is Ownable, IAsset {
    ...
    function updatePool(uint256 _pid) public {
        ...
        bool minRet = ok.mint(address(this), okReward);
        if (minRet) {
            pool.accOkPerShare = pool.accOkPerShare.add(okReward.mul(1e12).div(lpSupply));
            pool.allocOkAmount = pool.allocOkAmount.add(okReward);
            pool.accOkAmount = pool.allocOkAmount.add(okReward);
        }
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pendingAmount = user.amount.mul(pool.accOkPerShare).div(1e12).sub(user.rewardDebt);
            if (pendingAmount > 0) {
                safeOkTransfer(msg.sender, pendingAmount);
                user.accOkAmount = user.accOkAmount.add(pendingAmount);
                pool.allocOkAmount = pool.allocOkAmount.sub(pendingAmount);
            }
        }
        ...
        user.rewardDebt = user.amount.mul(pool.accOkPerShare).div(1e12);
    ...
    function pendingOk(uint256 _pid, address _user) public view returns (uint256) {
        ...
        if (user.amount > 0) {
            if (block.number > pool.lastRewardBlock) {
                uint256 blockReward = getOkBlockReward(pool.lastRewardBlock);
                uint256 okReward = blockReward.mul(pool.allocPoint).div(totalAllocPoint);
                accOkPerShare = accOkPerShare.add(okReward.mul(1e12).div(lpSupply));
                return user.amount.mul(accOkPerShare).div(1e12).sub(user.rewardDebt);
            }
            if (block.number == pool.lastRewardBlock) {
                return user.amount.mul(accOkPerShare).div(1e12).sub(user.rewardDebt);
            }
    ...
    function getPoolView(uint256 pid) public view returns (PoolView memory) {
        ...
        ERC20 token0 = ERC20(IOKswapPair(lpToken).token0());
        ERC20 token1 = ERC20(IOKswapPair(lpToken).token1());
        string memory symbol0 = token0.symbol();
        string memory name0 = token0.name();
        uint8 decimals0 = token0.decimals();
        string memory symbol1 = token1.symbol();
...
contract DepositPool is Ownable, IAsset {
    ...
    function updatePool(uint256 _pid) public {
        ...
        bool minRet = ok.mint(address(this), okReward);
        if (minRet) {
            pool.accOkPerShare = pool.accOkPerShare.add(okReward.mul(1e12).div(tokenSupply));
            pool.allocOkAmount = pool.allocOkAmount.add(okReward);
            pool.accOkAmount = pool.allocOkAmount.add(okReward);
        }
        pool.lastRewardBlock = block.number;
    ...
    function pendingOk(uint256 _pid, address _user) public view returns (uint256) {
        ...
        if (user.amount > 0) {
            if (block.number > pool.lastRewardBlock) {
                uint256 blockReward = getOkBlockReward(pool.lastRewardBlock);
                uint256 okReward = blockReward.mul(pool.allocPoint).div(totalAllocPoint);
                accOkPerShare = accOkPerShare.add(okReward.mul(1e12).div(tokenSupply));
                return user.amount.mul(accOkPerShare).div(1e12).sub(user.rewardDebt);
            }
            if (block.number == pool.lastRewardBlock) {
                return user.amount.mul(accOkPerShare).div(1e12).sub(user.rewardDebt);
...
contract OKswapAggregator {
    ...
    function getUserMiningInfos(address _account) public view returns (UserMiningInfo[] memory) {
        ...
        for (uint256 i = 0; i < liquidityPool.getPoolLength(); i++) {
            ...
            uint256 unclaimedRewards = liquidityPool.pendingOk(i, _account);
            if (unclaimedRewards > 0 || lpUV.accOkAmount > 0) {
                infos[index] = UserMiningInfo({
                    userAmount: lpUV.stakedAmount,
                    userUnclaimedReward: unclaimedRewards,
                    userAccReward: lpUV.accOkAmount,
                    poolType: 1,
                    pid: i,
                    pair: lpPV.lpToken,
        ...
        for (uint256 i = 0; i < tradingPool.getPoolLength(); i++) {
            ...
            uint256 unclaimedRewards = tradingPool.pendingOk(i, _account);
            if (unclaimedRewards > 0 || tUV.accOkAmount > 0) {
                infos[index] = UserMiningInfo({
                    userAmount: tUV.quantity,
                    userUnclaimedReward: tUV.unclaimedRewards,
                    userAccReward: tUV.accOkAmount,
                    poolType: 2,
                    pid: i,
                    pair: tPV.pair,
        ...
        for (uint256 i = 0; i < depositPool.getPoolLength(); i++) {
            ...
            uint256 unclaimedRewards = depositPool.pendingOk(i, _account);
            if (unclaimedRewards > 0 || dUV.accOkAmount > 0) {
                infos[index] = UserMiningInfo({
                    userAmount: dUV.stakedAmount,
                    userUnclaimedReward: dUV.unclaimedRewards,
                    userAccReward: dUV.accOkAmount,
                    poolType: 3,
                    pid: i,
                    pair: address(0),
